...
type family All  (as::[Bool]) :: Bool where
   All '[] = True
   All (True ':as) = All as
   All (False ':bs) = False
...
type family Maximum (as :: [Nat]) :: Nat where
  Maximum '[] = Z
  Maximum '[a] = a
...
type family InsTree (n :: Nat) (t :: Tree) :: Tree where
  InsTree n Empty = Node Empty n Empty
  InsTree n (Node a m b) = InsHelp (Compare n m) n (Node a m b)

type family InsHelp (o :: Ordering) (n :: Nat) (t :: Tree) :: Tree where
  InsHelp LT n (Node a m b) = Node (InsTree n a) m b
  InsHelp EQ n (Node a m b) = Node a m b
  InsHelp GT n (Node a m b) = Node a m (InsTree n b)

{- SIX -}

-- | Write a type family to /delete/ a promoted 'Nat' from a promoted 'Tree'.

-- type family DelTree (n::Nat) (t :: Tree) :: Tree where
--   DelTree n Empty = Empty
--   DelTree n (Node l x r) = DelHelp (Compare n x) n (Node l x r)

-- type family DelHelp (o::Ordering) (n::Nat) (t :: Tree) :: Tree where
--   DelHelp LT n (Node l x r) = Node (DelTree n l) x r
--   DelHelp GT n (Node l x r) = Node l x (DelTree n r)
--   DelHelp EQ n (Node Empty x r) = r
--   DelHelp EQ n (Node l x Empty) = l
--   DelHelp EQ n (Node l x r) = l -- TODO need to find the largest elem in l, put it in place of x and fix potentially broken l
...
deleteTest1 :: Delete 'Z (InsTree 'Z 'Empty) :~: 'Empty
deleteTest1 = Refl

deleteTest2 :: InsTree 'Z (InsTree 'Z 'Empty) :~: InsTree 'Z 'Empty
deleteTest2 = Refl

deleteTest3
   :: InsTree ('S 'Z) (InsTree 'Z 'Empty)
...
appHLs :: HList xs -> HList ys -> HList (xs ++ ys)
appHLs HNil ys         = ys
appHLs (HCons x xs) ys = HCons x (appHLs xs ys)
...
-- input natural.
type family NatsUpTo (n :: Nat) :: [Nat] where
  NatsUpTo n = Reverse (CountDown n)

type family CountDown (n :: Nat) :: [Nat] where
  CountDown Z = '[Z]
...
type family RevHelp (xs :: [Nat]) (ys :: [Nat]) :: [Nat] where
  RevHelp '[] ys = ys
...
upToTest :: NatsUpTo (S (S Z)) :~~~: '[Z, S Z, S (S Z)]
upToTest = Refle
