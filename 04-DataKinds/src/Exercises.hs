...
-- want.
newtype Imo (m :: IntegerMonoid) = Imo Integer

-- | b. Write the two monoid instances for 'Integer'.
instance Semigroup (Imo 'Sum) where
  Imo a <> Imo b = Imo (a + b)

instance Monoid (Imo 'Sum) where
  mempty = Imo 0

instance Semigroup (Imo 'Product) where
  Imo a <> Imo b = Imo (a * b)

instance Monoid (Imo 'Product) where
  mempty = Imo 1

-- | c. Why do we need @FlexibleInstances@ to do this?
-- ANSWER: Because `Imo Product` / `Imo Sum` is not of the form `T a` with a being type variable.
...
-- 'Void'?
-- ANSWER: No, becasue there are no promoted data constructors of kind Void.

-- | b. What are the possible type-level values of kind 'Maybe Void'?
-- ANSWER: Nothing :: Maybe Void
...
data StringAndIntList (stringCount :: Nat) where
  ...
  ConsString :: String -> StringAndIntList a -> StringAndIntList ('S a)
  ConsInt :: Int -> StringAndIntList a -> StringAndIntList a

-- | b. Update it to keep track of the count of strings /and/ integers.
data StringAndIntList2 (stringCount :: Nat) (intCount :: Nat) where
  S2Nil :: StringAndIntList2 'Z 'Z
  S2ConsString :: String -> StringAndIntList2 a b -> StringAndIntList2 (S a) b
  S2ConsInt :: Int -> StringAndIntList2 a b -> StringAndIntList2 a (S b)

-- | c. What would be the type of the 'head' function?
head :: StringAndIntList2 stringCount intCount -> Maybe (Either String Int)
head sail = case sail of
  S2Nil -> Nothing
  S2ConsString s _ -> Just (Left s)
  S2ConsInt i _ -> Just (Right i)
...
-- version of 'Bool'!
data BlogAction2 (adminOnly :: Bool) where
  AddBlog2 :: BlogAction2 'False
  DeleteBlog2 :: BlogAction2 'True
  AddComment2 :: BlogAction2 'False
  DeleteComment2 :: BlogAction2 'True

-- | b. Write a 'BlogAction' list type that requires all its members to be
-- the same "access level": "admin" or "non-admin".
data BlogActionList (isSafe :: Bool) where
  BANil :: BlogActionList isSafe
  BACons :: BlogAction2 isSafe -> BlogActionList isSafe -> BlogActionList isSafe

newtype BlogActionList2 (isSafe :: Bool)
  = BlogActionList2 [BlogAction2 isSafe]
...
data BlogAction3 (roles :: [Role]) where
  AddBlog3 :: BlogAction3 '[User, Moderator, Admin]
  DeleteBlog3 :: BlogAction3 '[Admin]
  AddComment3 :: BlogAction3 '[User, Moderator, Admin]
  DeleteComment3 :: BlogAction3 '[Moderator, Admin]
...
data SNat (value :: Nat) where
  SZero :: SNat Z
  SSucc :: SNat x -> SNat (S x)

-- | b. Write a function that extracts a vector's length at the type level:
length :: Vector n a -> SNat n
length v = case v of
  VNil -> SZero
  VCons _ v1 -> SSucc (length v1)
...
-- intentions:
data Program (state :: Bool) result where
...
interpret :: Program isOpen a -> IO a
interpret p = case p of
  OpenFile next -> putStrLn "Opened" >> interpret next
  WriteFile str next -> putStrLn ("Writing " <> str) >> interpret next
  ReadFile next -> interpret $ next "File contents"
  CloseFile next -> putStrLn "closing file" >> interpret next
  Exit a -> putStrLn "Exiting" >> pure a
...
(VCons a _) !! STZ = a
(VCons _ as) !! (STSucc n) = as !! n

-- | c. Write a function that converts a @SmallerThan n@ into a 'Nat'.
smallerThanToNat :: SmallerThan (n :: Nat) -> Nat
smallerThanToNat st = case st of
  STZ -> Z
  STSucc x -> S (smallerThanToNat x)
